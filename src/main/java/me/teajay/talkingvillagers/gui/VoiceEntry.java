...
public class VoiceEntry extends SpruceContainerWidget {
    ...
    public VoiceEntry(Voicepack voicepack, VoiceFile voice, Position position, int width, int height, Lambda callBack) {
        ...
        this.addChild(new SpruceLabelWidget(Position.of(position, 40, 2), Text.literal("▶ " + voice.getName()), width/3));
        var symbol = "?";
        ...
        Text text = (voice.isInstalled()) ? Text.literal("Uninstall") : Text.literal("Install");
        this.addChild(new SpruceLabelWidget(Position.of(position, width/3+40, 2), Text.literal(symbol), width/3-40));
